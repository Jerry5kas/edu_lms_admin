<x-layouts.main>
    <div class="bg-white rounded-2xl shadow p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Admin User: {{ $user->name }}</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="country_code" 
                            name="country_code" 
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="US" {{ old('country_code', $user->country_code) == 'US' ? 'selected' : '' }}>United States (US)</option>
                        <option value="CA" {{ old('country_code', $user->country_code) == 'CA' ? 'selected' : '' }}>Canada (CA)</option>
                        <option value="GB" {{ old('country_code', $user->country_code) == 'GB' ? 'selected' : '' }}>United Kingdom (GB)</option>
                        <option value="DE" {{ old('country_code', $user->country_code) == 'DE' ? 'selected' : '' }}>Germany (DE)</option>
                        <option value="IN" {{ old('country_code', $user->country_code) == 'IN' ? 'selected' : '' }}>India (IN)</option>
                        <option value="AU" {{ old('country_code', $user->country_code) == 'AU' ? 'selected' : '' }}>Australia (AU)</option>
                    </select>
                    @error('country_code') 
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p> 
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="timezone" 
                        ...
                        <option value="Europe/Berlin" {{ old('timezone', $user->timezone) == 'Europe/Berlin' ? 'selected' : '' }}>Berlin (CET)</option>
