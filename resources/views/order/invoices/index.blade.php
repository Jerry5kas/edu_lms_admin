...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex space-x-3">
            ...
            <a href="{{ route('invoices.index', ['export' => 'csv']) }}"
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">₹{{ number_format($stats['total_invoiced_amount'], 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['invoices_this_month'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['invoices_this_year'] }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('invoices.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="order_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">All Orders</option>
                        @foreach($orders as $order)
                            <option value="{{ $order->id }}" {{ request('order_id') == $order->id ? 'selected' : '' }}>
                                #{{ $order->id }} - {{ $order->user->name }} - ₹{{ number_format($order->total_cents / 100, 2) }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-end">
                    ...
                    <a href="{{ route('invoices.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
                        Clear
                    </a>
                </div>
            </form>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div>
                                ...
                                <div class="text-sm text-gray-500">{{ $invoice->order->gateway_order_id }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">₹{{ number_format($invoice->order->total_cents / 100, 2) }}</div>
                            <div class="text-sm text-gray-500">{{ $invoice->order->currency }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $invoice->issued_at->format('M d, Y H:i') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                ...
                                <form method="POST" action="{{ route('invoices.regenerate-pdf', $invoice) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="text-yellow-600 hover:text-yellow-900"
                                            onclick="return confirm('Are you sure you want to regenerate the PDF?')">
                                        Regenerate
                                    </button>
                                </form>
