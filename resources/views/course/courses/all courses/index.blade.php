            ...
            <div class="bg-white rounded-2xl shadow p-4 flex flex-col transition hover:shadow-xl">
                ...
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <p class="text-sm text-gray-600">
                        Created by <span class="font-semibold text-gray-800">{{ Auth::user()->name ?? 'N/A' }}</span>
                    </p>
                </div>
                ...
                <div class="mb-3">
                    @if($course->is_published)
                        <span class="inline-block px-2 py-1 text-xs rounded-full bg-green-100 text-green-600 font-medium">
                            Published
                        </span>
                    @else
                        <span class="inline-block px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-600 font-medium">
                            Draft
                        </span>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('courses.show', $course) }}"
                           class="px-3 py-1 text-xs font-medium text-blue-600 border border-blue-600 rounded-full hover:bg-blue-50 transition">
                            View Details
                        </a>
                        <!-- <a href="{{ route('courses.sections.index', $course) }}"
                           class="px-3 py-1 text-xs font-medium text-green-600 border border-green-600 rounded-full hover:bg-green-50 transition">
                            Sections
                        </a> -->
{{--                        @if($course->is_published)--}}
{{--                            <form action="{{ route('courses.unpublish', $course) }}" method="POST" class="inline">--}}
{{--                                @csrf--}}
{{--                                @method('PATCH')--}}
{{--                                <button type="submit" class="px-3 py-1 text-xs font-medium text-orange-600 border border-orange-600 rounded-full hover:bg-orange-50 transition">--}}
{{--                                    Unpublish--}}
{{--                                </button>--}}
{{--                            </form>--}}
{{--                        @else--}}
{{--                            <form action="{{ route('courses.publish', $course) }}" method="POST" class="inline">--}}
{{--                                @csrf--}}
{{--                                @method('PATCH')--}}
{{--                                <button type="submit" class="px-3 py-1 text-xs font-medium text-green-600 border border-green-600 rounded-full hover:bg-green-50 transition">--}}
{{--                                    Publish--}}
{{--                                </button>--}}
{{--                            </form>--}}
{{--                        @endif--}}
