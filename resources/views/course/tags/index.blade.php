    ...
    <div class="bg-white p-6">
        <div class="w-full max-w-7xl mx-auto" x-data="{ open:false, selected:'Sort Order' }">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                <!-- Heading -->
                <h1 class="text-2xl font-bold text-gray-800">Course Tags</h1>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-3">
                ...
                <div class="text-gray-500 text-sm">
                    Home / <span class="text-gray-900 font-medium">Course Tags</span>
                </div>
                ...
                <table class="w-full text-sm text-left text-gray-600">
                    ...
                    @foreach($tags as $index => $tag)
                        <tr>
                            ...
                            <td class="border px-3 py-2">{{ $tag->name }}</td>
                            <td class="border px-3 py-2">{{ $tag->slug }}</td>
                            <td class="border px-3 py-2">{{ $tag->courses_count }}</td>
                            <td class="border px-3 py-2">{{ $tag->created_at }}</td>
            ...
            <div class="flex justify-between items-center mt-4 text-sm text-gray-600">
                <p>Showing 1 to {{ $tags->count() }} of {{ $tags->count() }} entries</p>
            </div>
        </div>
    </div>
</x-layouts.main>
